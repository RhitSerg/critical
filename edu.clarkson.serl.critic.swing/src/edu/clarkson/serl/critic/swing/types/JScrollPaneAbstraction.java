 ...
 *
 */
public class JScrollPaneAbstraction extends JComponentAbstraction {
	public static final String TYPE = "javax.swing.JScrollPane";
	public static final Value DEFAULT_VALUE = AbstractValue.fromObject(TYPE);

	public JScrollPaneAbstraction() {
		super(DEFAULT_VALUE);
	}

	public JScrollPaneAbstraction(Value sootValue) {
		super(sootValue);
	}

	public JScrollPaneAbstraction(Value sootValue, boolean open, boolean mutable) {
	...
	public IResult execute(InvokeExpr invokeExpr, List<ISymbol<? extends Value>> arguments) {
		...
		else if(method.equals("setViewportView")) {
			return this.setViewportView(arguments);
		}
		else if(method.equals("getViewport")) {
	...
	public IResult init(List<ISymbol<? extends Value>> arguments) {
		...
		for(ISymbol<? extends Value> arg : arguments) {
			// If the constructor has a JComponent then add it as its children
			if(arg instanceof JComponentAbstraction) {
	...
	public IResult setViewportView(List<ISymbol<? extends Value>> arguments) {
		ISymbol<? extends Value> view = arguments.get(0);
		if(view instanceof JComponentAbstraction) {
			this.add(arguments);
		}
